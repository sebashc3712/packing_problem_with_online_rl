...
class BoxPilingEnv:
    ...
    # Convenience: can we place this box anywhere right now?
    def can_place_box(self, box_dims):
        valid_actions, _ = self.get_valid_actions(box_dims)
...
# ---------------------
def train(episodes_boxes, output_dir, verbose=False, episode_to_show=100, defer_penalty=-5.0):
    """
    ...
    for episode in range(total_episodes):
        ...
        buffer = []  # boxes that didn't fit on arrival
        placements_this_episode = 0
        attempts_this_episode = 0  # how many times we tried to place a box (initial + buffer)

        # Helper that tries to place a single box once, using the agent-selected heuristic.
        # Returns (placed: bool, reward: float)
        def try_place_one_box(box_dims):
            ...
            if verbose and episode == episode_to_show:
                ...
                print(f"BoxAttempt: {attempts_this_episode:02d} | Heuristic: {heuristic} | Action: {mapping}")

            if action is None:
                # Defer: give a small penalty (can set to 0.0 if undesired)
                agent.remember(state, heuristic_index, defer_penalty, state, False)
                agent.replay()
            ...
            agent.remember(state, heuristic_index, reward, next_state, local_done)
            agent.replay()
            agent.update_target_model()
            state = next_state
        ...
        # 1) First pass: stream of incoming boxes
        while not done and box_idx < len(boxes):
            ...
            if not placed:
                buffer.append(box_dims)  # Defer this box

            # Early stop if the pallet is already full by height rule
            if env._is_terminal():
                done = True
                break

        # 2) Process the buffer until empty or no progress in a full pass
        while not done and len(buffer) > 0:
            made_progress = False
            new_buffer = []
            for box_dims in buffer:
                # If there is no valid placement at all right now, keep it for the next pass
            ...
            if not made_progress:
                # No box from the buffer could be placed in this pass → stop
                done = True
                break
        ...
        episode_metrics = {
            ...
            'boxes_attempted': attempts_this_episode,     # total placement attempts (initial + buffer)
        ...
        # Epsilon decay & LR schedule tick (only if any optimizer step happened)
        ...
        print(f"Episode: {episode+1:04d} | Util: {utilization:.1%} | "
              ...
              f"Placed: {len(env.placed_boxes):02d} | Attempts: {attempts_this_episode:02d} | "
...
if __name__ == "__main__":
    ...
    final_metrics = train(episodes_boxes, output_dir=output_directory, verbose=False, episode_to_show=100)
