...
def make_env():
    from oskp_rl_up_buffer_experiments_v3 import BoxPilingEnv
    return BoxPilingEnv()

def parallel_train_one_epoch(agent, episodes_boxes, output_dir, env_params, n_envs=8):
    from vec_env_v3 import SubprocVecEnv

    # --- Configuration ---
    max_buffer_size = env_params.get('max_buffer_size', 0)
    defer_penalty = -0.5

    # Scale batch size to match sequential learning power
    ...
    while any(not d for d in slot_done) or next_ep_idx < n_episodes:
        ...
        # Pass actual buffer contents (list of box dims) instead of just count
        ...
        for idx, slot_idx in enumerate(step_envs_indices):
            ...
            if action is None:
                agent.remember(state, h_idx, defer_penalty, state, True, buffer_boxes, proxy_scores)
