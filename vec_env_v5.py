...
def worker(remote, parent_remote, env_fn):
    parent_remote.close()
    from oskp_rl_up_buffer_experiments_v5 import proxy_scores_for_heuristics
    ...
    try:
        while True:
            ...
            if cmd == 'step':
                # data can be (action,) or (action, free_buffer_slots, max_buffer_slots)
                if isinstance(data, tuple):
                    action, free_buf, max_buf = data
                    ob, reward, done, info = env.step(action, free_buffer_slots=free_buf, max_buffer_slots=max_buf)
                else:
                ...
                # data is (pred_mask_probs, buffer_boxes, max_buffer_slots)
                pred_mask_probs, buffer_boxes, max_buffer_slots = data
                scores = proxy_scores_for_heuristics(
                    env.current_height_map, env.current_box,
                    env.pallet_size, env.max_height, pred_mask_probs,
                    buffer_boxes=buffer_boxes,
            ...
            elif cmd == 'can_place_box':
                # data is box_dims
                result = env.can_place_box(data)
                remote.send(result)
