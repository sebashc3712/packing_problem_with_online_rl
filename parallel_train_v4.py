...
def make_env():
    from oskp_rl_up_buffer_experiments_v4 import BoxPilingEnv
    return BoxPilingEnv()

def parallel_train_one_epoch(agent, episodes_boxes, output_dir, env_params, n_envs=8):
    from vec_env_v4 import SubprocVecEnv
    ...
    pbar = tqdm(total=n_episodes, desc="Parallel Training v4", unit="ep")
        ...
        for idx, slot_idx in enumerate(step_envs_indices):
            ...
            if h_idx == 5:
                ...
                if len(slot_buffers[slot_idx]) < max_buffer_size:
                    # Successfully deferred to buffer
                    buffer_fill_ratio = len(slot_buffers[slot_idx]) / max_buffer_size if max_buffer_size > 0 else 1.0
                    reward = -0.05 - 0.1 * buffer_fill_ratio
                    slot_buffers[slot_idx].append(box_dims)
                    # done=True prevents self-referential Q-bootstrap (next_state == state)
