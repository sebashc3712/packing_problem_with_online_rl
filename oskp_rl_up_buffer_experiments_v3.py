...
# =====================================================
# DQN Agent (dueling + mask head, buffer DIMENSIONS input)
# =====================================================
class DQNAgent:
    def __init__(
        self,
        ...
        # Vector input size: 3 (box dims) + max_buffer_slots*3 (buffer box dims) + 5 (proxy scores)
        self.vec_input_size = 3 + max_buffer_slots * 3 + 5
    ...
    def _norm(self, state, buffer_boxes=None, proxy_scores=None):
        ...
        # [BoxL, BoxW, BoxH, BufBox1_L, BufBox1_W, BufBox1_H, ..., S1, S2, S3, S4, S5]
    ...
    def _build_model(self):
        ...
        class ProNet(nn.Module):
            def __init__(self, pallet_size, action_size, vec_in_size):
                ...
                # Branch B: Vector (Box + Buffer Dims + Scores)
                self.vector = nn.Sequential(
                    nn.Linear(vec_in_size, 64), nn.ReLU(),
    ...
    # --- Methods updated for buffer_boxes ---
    ...
    def remember(self, s, a, r, ns, d, buffer_boxes, proxy_scores):
        ...
        # Store buffer_boxes as a list of arrays (deep copy)
...
def proxy_scores_for_heuristics(height_map, box_dims, pallet_size, max_height, pred_mask_probs):
    ...
    Stateless function (accepts params instead of env object).
    """
    scores = [0.0] * 5
